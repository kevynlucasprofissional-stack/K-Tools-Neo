...
class MegaPodcastApp(ctk.CTk):
    ...
    def _montar_ui(self):
        ...
        ctk.CTkLabel(config, text="Modo de velocidade:").grid(
            row=2, column=2, padx=(8, 16), pady=(0, 4), sticky="w"
        )

        self.combo_workers = ctk.CTkComboBox(
            config,
            values=[
                "1 - Normal",
                "2 - Rápido",
        ...
        self.combo_workers.set("4 - Turbo")
        self.combo_workers.grid(row=3, column=2, padx=(8, 16), pady=(0, 14))
    ...
    def iniciar(self):
        ...
        workers = int(self.combo_workers.get().split(" ", 1)[0])

        thread = threading.Thread(
            target=self._processar,
            args=(raiz, saida, self.combo_bitrate.get(), workers),
            daemon=True
        )
        thread.start()
    ...
    def _processar(self, raiz: Path, saida: Path, bitrate: str, workers: int):
        temp_dir = None

        try:
            ffmpeg = ffmpeg_exe()

            self.escrever_log("=== MEGA PODCAST — MODO PARALELO ===")
            self.escrever_log(f"Aulas: {TOTAL_AULAS}")
            self.escrever_log(f"Codec final: AAC {bitrate}")
            self.escrever_log(f"Processos simultâneos: {workers}")
            self.escrever_log(f"Sample rate: {SAMPLE_RATE} Hz")
            self.escrever_log(f"Canais: {CHANNELS}")
            self.escrever_log("")
            self.escrever_log(
                "As aulas são processadas em paralelo, mas serão juntadas "
                "na cronologia fixa original."
            )
            self.escrever_log("")
            ...
            segmentos_dir = temp_dir / "segmentos"
            segmentos_dir.mkdir(parents=True, exist_ok=True)

            concluido = 0

            def extrair_aula(idx: int, rel: str):
                if self.cancelar:
                    return idx, rel, None, "cancelado"

                video = caminho_real(raiz, rel)
                destino = segmentos_dir / f"{idx:04d}.aac"
                ...
                if proc.returncode != 0:
                    ...
                    return idx, rel, None, erro or "erro desconhecido"

                if not destino.exists() or destino.stat().st_size == 0:
                    return idx, rel, None, "arquivo de áudio vazio"

                return idx, rel, destino, None

            self.status(
                f"Extraindo {TOTAL_AULAS} aulas com {workers} processos simultâneos…",
                0.0
            )
            ...
            with ThreadPoolExecutor(max_workers=workers) as executor:
                for idx, rel in enumerate(ORDEM_AULAS, start=1):
                    future = executor.submit(extrair_aula, idx, rel)
                ...
                for future in as_completed(futures):
                    ...
                    idx, rel, destino, erro = future.result()
                    ...
                    resultados[idx] = destino
                    concluido += 1
                    fracao = concluido / (TOTAL_AULAS + 1)

                    self.status(
                        f"Extraídas {concluido}/{TOTAL_AULAS} aulas "
                        f"({workers} simultâneas)…",
                        fracao
                    )

                    self.escrever_log(
                        f"✓ [{idx:03d}/{TOTAL_AULAS}] {rel}"
                    )

            if self.cancelar:
                raise InterruptedError("Processamento cancelado pelo usuário.")

            # Junta os segmentos pela ORDEM_AULAS, não pela ordem em que terminaram.
            self.status(
                "Juntando os áudios na cronologia correta…",
            ...
            self.escrever_log("")
            self.escrever_log("=== CONCATENAÇÃO CRONOLÓGICA ===")

            with combinado_aac.open("wb") as saida_aac:
                for idx in range(1, TOTAL_AULAS + 1):
                    segmento = resultados[idx]

                    with segmento.open("rb") as entrada:
                        shutil.copyfileobj(
                            entrada,
                            saida_aac,
                            length=1024 * 1024
            ...
            # Remux AAC -> M4A, sem recodificar.
            self.status(
                "Finalizando M4A sem recompressão…",
                TOTAL_AULAS / (TOTAL_AULAS + 1)
            )

            self.escrever_log("")
            self.escrever_log("Empacotando AAC em M4A (stream copy)…")
            ...
            self.escrever_log(f"Paralelismo usado: {workers} processos")
            ...
            self.after(
                0,
                lambda msg=str(e): messagebox.showerror("Erro", msg)
            )
