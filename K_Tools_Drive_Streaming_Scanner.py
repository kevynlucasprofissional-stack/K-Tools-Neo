...
APP_VERSION = "1.0.0"
CHECKPOINT_NAME = "drive_structure_checkpoint.sqlite3"
...
class DriveTreeScanner:
    def __init__(
        ...
        self.db_path = self.output_dir / CHECKPOINT_NAME
    ...
    def run(self, resume: bool = True) -> dict:
        ...
        try:
            ...
            while True:
                ...
                row = conn.execute(
                    "SELECT id,rel_path,depth,attempts FROM nodes WHERE scan_state='pending' ORDER BY id LIMIT 1"
                ).fetchone()
                ...
                if row is None:
                    ...
                    if error_count and retry_round < self.options.retry_rounds:
                        ...
                        conn.commit()
                        self._emit(message=f"Revisitando {error_count} pasta(s) com erro — rodada {retry_round}...", **self._stats(conn))
                ...
                try:
                    entries, stable, attempts_used = enumerate_stable(path, self.options, self.cancel_event)
                    ...
                    self._emit(
                        message=f"Lida: {pretty} | +{dirs_added} pasta(s)" + (f" | +{files_added} arquivo(s)" if self.options.include_files else ""),
                        current_path=pretty,
                        **stats,
                    )
                except ScanCancelled:
                    ...
                    conn.commit()
                    self._emit(message=f"Falha temporária/permanente: {pretty} — {exc}", current_path=pretty, **self._stats(conn))
            ...
            outputs = export_reports(conn, self.root, self.output_dir, self.options)
...
def export_reports(conn: sqlite3.Connection, root: Path, output_dir: Path, options: ScanOptions) -> dict[str, Path]:
    output_dir.mkdir(parents=True, exist_ok=True)
    txt_path = output_dir / "estrutura_drive.txt"
    csv_path = output_dir / "pastas_drive.csv"
    json_path = output_dir / "estrutura_drive.json"
    errors_path = output_dir / "diagnostico_drive.csv"
    summary_path = output_dir / "resumo_drive.json"
    files_path = output_dir / "arquivos_drive.csv"
...
class ScannerApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Checkbutton(opts, text="Incluir pastas/arquivos ocultos", variable=self.include_hidden_var).grid(row=1, column=0, sticky="w", pady=(6, 0))
        ttk.Checkbutton(opts, text="Inventariar também arquivos (mais lento; árvore de pastas não precisa disso)", variable=self.include_files_var).grid(row=2, column=0, sticky="w", pady=(6, 0))
        ttk.Checkbutton(opts, text="Retomar checkpoint existente quando possível", variable=self.resume_var).grid(row=3, column=0, sticky="w", pady=(6, 0))
    ...
    def start_scan(self) -> None:
        ...
        self.scanner = DriveTreeScanner(Path(root), Path(out), options, self._on_progress)
