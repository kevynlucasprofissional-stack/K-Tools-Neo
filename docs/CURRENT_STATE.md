...
## M0 â€” Platform Foundation â€” RESOLVED / PROMOTED
...
## M1 â€” First official Node Pack â€” RESOLVED
...
## AG-001 â€” xyflow interaction spike â€” CLOSED
...
## M2 â€” Durable Execution V1 â€” RESOLVED
...
## M3 â€” Diagnostics + Support Bundle â€” RESOLVED / PROMOTED
...
## M4 â€” Artifact Lifecycle + Recovery + Semantic Cache V1 â€” RESOLVED / PROMOTED
...
WorkflowEngine
  â”œâ”€ RunJournal
  â”œâ”€ DiagnosticsSession
  â”œâ”€ NodeCache
  â””â”€ ArtifactRegistry
```
...
- aggregate single-output publication and multi-output publication are distinct transaction boundaries and must not be generalized merely because both use tempâ†’promote mechanics.

## M5 â€” Official local Node Packs â€” ACTIVE / ITERATIVE DELIVERY

### Slice 1 â€” Text Node Pack V1 â€” RESOLVED / PROMOTED
...
### Slice 2 â€” PDF Merge Node Pack V1 â€” RESOLVED / PROMOTED
...
### Slice 3 â€” PDF Split Node V1 â€” RESOLVED / PROMOTED

`packages/ktools-pdf/` is canonical for balanced PDF split. `file.literal`, `pdf.split.parts: FILE -> FILE_SET` v1 NEVER, balanced page planning, collision-safe per-part atomic publication, explicit partial-set failure, PDF Artifact page metadata, snapshots, cache/publication proof and splitâ†’merge composition are hosted-tested.

Terminal closure `a26dfcee626eedc27366dfec93be68503343941a`, run `33656157870`, 5/5.

### Slice 4 â€” Text Split Node V1 â€” RESOLVED / PROMOTED

`packages/ktools-text/` is canonical for balanced Markdown/TXT split. Split-specific decode policy, balanced line planning, UTF-8 collision-safe per-part publication, `text.split.parts: FILE -> FILE_SET` v1 NEVER, Artifact metadata/snapshots, cached-source/republication proof and splitâ†’merge composition are hosted-tested.
...
### Slice 5 â€” Mixed Document Split Orchestrator V1 â€” RESOLVED / PROMOTED
...
### Slice 6 â€” Image Safety Foundation + WebPâ†’PNG V1 â€” RESOLVED / PROMOTED

`packages/ktools-images/` is the canonical evolution owner for the image-safety foundation and WebPâ†’PNG conversion.
...
The stable GUI WebPâ†’PNG path is compatibility debt.

### Slice 7 â€” Imagesâ†’PDF Node V1 â€” RESOLVED / PROMOTED

Fresh terminal-main discovery compared Imagesâ†’PDF and bounded Files/Folders and selected Imagesâ†’PDF only after the Slice-6 image foundation made its remaining contract bounded. Files/Folders remained deferred pending a dedicated cross-platform traversal/result contract.

Delivered:

- shared `ktools_images.reader` as single guarded Pillow decode / bomb / frame-0 / EXIF owner;
- WebPâ†’PNG refactored onto that shared reader with zero observed semantic regression;
- supported JPG/JPEG/PNG/WebP/BMP/TIF/TIFF filtering in preserved compatible-source order;
- one first-frame source â†’ one PDF page;
- EXIF normalization before page preparation;
- RGB page normalization with RGBA/LA/palette transparency composited over white;
- one aggregate PDF with same-directory tempâ†’promote publication and prior-destination preservation on handled failure;
- canonical Imagesâ†’PDF writer shared by direct API and workflow;
...
- cached PURE `files.literal` without suppressed Imagesâ†’PDF publication;
...
The legacy stable GUI Imagesâ†’PDF implementation is compatibility debt; semantic evolution belongs to `ktools-images`.

### Slice 8 â€” Folder Scan Node V1
...
### Slice 9 — Media Extract Audio Node V1 — RESOLVED / PROMOTED

Status: **RESOLVED**

packages/ktools-media/ is canonical for FFmpeg-based audio extraction. media.extract_audio, atomic publication, M3 observability compliance via 
ecord_subprocess, cross-platform binaries via imageio-ffmpeg, and stream probe fallback are proved.

ADR: docs/decisions/ADR-031-MEDIA-EXTRACT-AUDIO-NODE-V1.md.
Evidence/final report: docs/specs/media-extract-audio-node-v1/.
