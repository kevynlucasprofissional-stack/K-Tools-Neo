...
Status: **PROVED / ACCEPTED PENDING FINAL MEMORY GATE**
Milestone: M5 Slice 2

## Decision

`packages/ktools-pdf/` is the canonical evolution owner for PDF merge behavior.

The V1 workflow node is:

`pdf.merge.files: FILE_SET -> PDF`

with version `1` and `CachePolicy.NEVER`.
...
3. V1 reuses ordered `FILE_SET`; a new `PDF_SET` is deferred until a second real collection contract proves it necessary.
...
- Accepted technical candidate: `a370028b9dbb2c44981a3c7e05d176ce7e54b71c`, run `33649789491` 5/5 including real PDF workflow smoke in Windows/Linux lanes.

## Legacy boundary

`K Tools Neo - Versão Estável 2.py` still contains historical PDF merge logic. After final memory promotion it is a frozen compatibility path, not an independent semantic owner. New PDF merge fixes/features originate in `ktools-pdf`; later traditional Tool/UI migration must redirect or retire the historical path.
