...
class App(QMainWindow):
    ...
    def criar_interface(self):
        ...
        # COLUNA DIREITA
        # A coluna usa uma área rolável para que os cards nunca sejam
        # comprimidos verticalmente em telas menores ou com escala alta.
        direita_scroll = QScrollArea()
        direita_scroll.setObjectName("rightScroll")
        direita_scroll.setWidgetResizable(True)
        direita_scroll.setFrameShape(QFrame.NoFrame)
        direita_scroll.setHorizontalScrollBarPolicy(Qt.ScrollBarAlwaysOff)
        direita_scroll.setMinimumWidth(500)

        direita_conteudo = QWidget()
        direita_conteudo.setObjectName("rightContent")
        direita = QVBoxLayout(direita_conteudo)
        direita.setContentsMargins(0, 0, 4, 0)
        direita.setSpacing(14)
        direita_scroll.setWidget(direita_conteudo)
        corpo.addWidget(direita_scroll, 2)
        ...
        config_layout.addWidget(self.combo_bitrate)

        # Informação de formato em largura total: evita o antigo card
        # apertado ao lado do combo de bitrate.
        modo_box = QFrame()
        modo_box.setObjectName("fastMode")
        modo_layout = QHBoxLayout(modo_box)
        modo_layout.setContentsMargins(12, 9, 12, 9)
        modo_layout.setSpacing(10)
        modo_tag = QLabel("MODO RÁPIDO")
        modo_tag.setObjectName("fastTag")
        modo_tag.setMinimumWidth(86)
        modo_desc = QLabel("AAC 48 kHz estéreo • união final por stream copy, sem recodificar tudo novamente.")
        modo_desc.setObjectName("fastDesc")
        modo_desc.setWordWrap(True)
        modo_layout.addWidget(modo_tag, 0, Qt.AlignVCenter)
        modo_layout.addWidget(modo_desc, 1)
        config_layout.addWidget(modo_box)
        ...
        self.combo_motor.currentTextChanged.connect(self.atualizar_explicacao_motor)
        config_layout.addWidget(self.combo_motor)

        # Explicação dinâmica do motor selecionado
        self.motor_info = QFrame()
        self.motor_info.setObjectName("motorInfo")
        motor_info_layout = QVBoxLayout(self.motor_info)
        motor_info_layout.setContentsMargins(12, 10, 12, 10)
        motor_info_layout.setSpacing(4)

        motor_topo = QHBoxLayout()
        motor_topo.setSpacing(8)
        self.motor_info_nome = QLabel("")
        self.motor_info_nome.setObjectName("motorInfoName")
        self.motor_info_badge = QLabel("")
        self.motor_info_badge.setObjectName("motorInfoBadge")
        self.motor_info_badge.setAlignment(Qt.AlignCenter)
        motor_topo.addWidget(self.motor_info_nome)
        motor_topo.addStretch()
        motor_topo.addWidget(self.motor_info_badge)

        self.motor_info_desc = QLabel("")
        self.motor_info_desc.setObjectName("motorInfoDesc")
        self.motor_info_desc.setWordWrap(True)

        motor_info_layout.addLayout(motor_topo)
        motor_info_layout.addWidget(self.motor_info_desc)
        config_layout.addWidget(self.motor_info)

        self.atualizar_explicacao_motor(self.combo_motor.currentText())
        ...
        self.label_saida.setMinimumHeight(38)
        self.label_saida.setTextInteractionFlags(Qt.TextSelectableByMouse)
    ...
    def aplicar_estilo(self):
        self.setStyleSheet("""
            ...
            QLabel#motorInfoName {
                color: #E8EAF1;
                font-size: 11px;
                font-weight: 700;
            }
            QLabel#motorInfoBadge {
                background: #231E3D;
                color: #B9AAFF;
                border: 1px solid #3B3265;
                border-radius: 8px;
                padding: 3px 7px;
                font-size: 8px;
                font-weight: 800;
            }
            QLabel#motorInfoDesc {
                color: #858EA1;
                font-size: 10px;
                line-height: 1.3;
            }
    ...
    def atualizar_explicacao_motor(self, motor):
        """Atualiza o resumo de UX do motor selecionado."""
        opcoes = {
            "Turbo adaptativo": (
                "Turbo adaptativo",
                "RECOMENDADO",
                "Escolhe automaticamente a estratégia mais eficiente para cada vídeo: "
                "reaproveita cache, usa Smart Copy quando possível, processa em paralelo "
                "e pode dividir vídeos longos. É a melhor opção para uso geral."
            ),
            "Paralelo + Smart Copy": (
                "Paralelo + Smart Copy",
                "MAIS VELOCIDADE",
                "Executa vários processamentos ao mesmo tempo e copia diretamente áudios "
                "AAC já compatíveis. Prioriza velocidade e funciona melhor em SSD/NVMe "
                "e computadores com vários núcleos."
            ),
            "Single-pass sem temporários": (
                "Single-pass sem temporários",
                "MENOS DISCO",
                "Processa a sequência em um fluxo contínuo, evitando criar vários arquivos "
                "temporários. Usa menos espaço e I/O de disco, mas normalmente aproveita "
                "menos paralelismo."
            ),
        }

        nome, badge, descricao = opcoes.get(
            motor,
            (motor, "", "Selecione um motor para ver como ele trabalha.")
        )
        self.motor_info_nome.setText(nome)
        self.motor_info_badge.setText(badge)
        self.motor_info_badge.setVisible(bool(badge))
        self.motor_info_desc.setText(descricao)
    ...
    def atualizar_preview_saida(self):
        ...
        caminho_saida = str(self.pasta / nome)
        self.label_saida.setText(caminho_saida)
        self.label_saida.setToolTip(caminho_saida)
