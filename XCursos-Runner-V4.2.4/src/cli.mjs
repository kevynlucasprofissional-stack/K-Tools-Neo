...
export async function login({configStore,runtime,url=null,playwrightLoader=null,launcherFactory=null,humanGate=null}){
  // Critical trust boundary: launch Chrome first and let the human finish Cloudflare/login BEFORE Playwright attaches.
  await openHumanChrome({runtime,launcherFactory,url:url||'https://www.xcursos.com/'});
  let rl=null;
  try{
    if(humanGate){
      await humanGate();
    ...
    const browser=new PlaywrightBrowser({profileDir:runtime.profileDir,cdpEndpoint:runtime.cdpEndpoint,playwrightLoader});
    try{
...
async function auditRecent(config){
  ...
  const store=new StateStore({outputRoot:config.outputRoot,courseName:recent.state.courseName,totalPositions:Number(recent.state.totalPositions)});
  await store.initialize({resume:true,workPageUrl:recent.state.workPageUrl});const dl=new MediaDownloader();await dl.preflight();const audit=await store.audit({validator:f=>dl.validateVideo(f)});return{ok:audit.healthyComplete,status:'AUDIT',course:recent.state.courseName,audit};
}
...
export async function main(argv=process.argv.slice(2),deps={}){
  ...
  if(parsed.command==='config'){
    ...
    const result={ok:true,status:'CONFIG',config};parsed.options.json?printJson(result):printHuman(result);return 0;
  }
  const runtime=runtimeConfig(config,parsed.options);
  const runnerRuntime={profileDir:runtime.profileDir,cdpEndpoint:runtime.cdpEndpoint,startUrl:runtime.startUrl,outputRoot:runtime.outputRoot,resume:runtime.resume,browser:deps.browser||null,downloader:deps.downloader||null,enableSignalHandlers:true,progressSink:deps.progressSink||((line)=>process.stderr.write(`${line}\n`))};
  let result;
  try{
    switch(parsed.command){
      case 'browser':result={ok:true,status:'BROWSER_READY',...(await openHumanChrome({runtime,launcherFactory:deps.launcherFactory,url:parsed.options.url||'https://www.xcursos.com/'}))};break;
      case 'login':result=await login({configStore,runtime,url:parsed.options.url||null,playwrightLoader:deps.playwrightLoader,launcherFactory:deps.launcherFactory,humanGate:deps.humanGate});break;
      ...
      case 'audit':result=await auditRecent({...config,outputRoot:runtime.outputRoot});break;
    ...
    parsed.options.json?printJson(result):printHuman(result);return result?.ok===false?2:0;
  }catch(error){const failure={ok:false,status:'ERROR',error:safeError(error),details:sanitizeForPersistence(error?.details||null)};parsed.options.json?printJson(failure):printHuman(failure);return 2;}
}
