...
const REFRESHABLE_SIGNED_MEDIA_FAILURES=new Set(['HTTP_403','HTTP_429','HTTP_5XX','NETWORK_RESET','NETWORK_TIMEOUT','DNS_ERROR','TLS_ERROR','PROCESS_TIMEOUT']);
...
export class XCursosCourseRunner {
  ...
  async processPosition(position){
    ...
    let lesson=await this.ensurePageAt(position);
    if(lesson.currentPosition!==position)throw new RunnerError(`Posição observada ${lesson.currentPosition}, esperada ${position}.`,{code:'POSITION_MISMATCH'});
    if(!sameCourse(lesson.courseName,this.courseName))throw new RunnerError(`Página mudou para outro curso: ${lesson.courseName}`,{code:'COURSE_IDENTITY_MISMATCH'});
    if(lesson.totalPositions!==this.total)throw new RunnerError(`TOTAL mudou de ${this.total} para ${lesson.totalPositions}.`,{code:'TOTAL_CHANGED'});
    await this.rememberNavigation(lesson);
    await this.logger.log(`LESSON ${position}/${this.total}`,'Inspecting',{lesson:lesson.lessonTitle,module:lesson.moduleName});
    ...
    let attempts=0, status, outputFile=null, validation=null, downloadFailure=null;
    ...
    else if(!lesson.videoUrl || lesson.mediaType==='NONE') { status=lesson.hasMaterialsLinks?'NO_VIDEO':'MEDIA_NOT_FOUND'; await this.logger.log(`LESSON ${position}/${this.total}`,'No downloadable lesson media found',{status}); }
    else {
      await fs.mkdir(paths.moduleDir,{recursive:true});
      if(!repair && !this.state.getInFlight(position)){
        await this.state.setInFlight({
          position,
          lessonTitle:lesson.lessonTitle,
          moduleName:lesson.moduleName,
          lessonUrl:lesson.pageUrl||this.workPage.url,
          relativeOutputBase:path.relative(this.state.courseDir,path.join(paths.moduleDir,paths.baseName)),
        });
      }
      ...
      if(existingFile){
        try{validation=await this.downloader.validateVideo(existingFile,{signal:this.shutdown.signal});status='ALREADY_PRESENT';outputFile=existingFile;await this.logger.log('VERIFY','Existing file valid',{position,duration:validation.duration});}
          ...
          if(typeof this.downloader.quarantineCorrupt==='function') quarantine=await this.downloader.quarantineCorrupt(existingFile);
            ...
            quarantine=`${existingFile}.corrupt-${Date.now()}`;
            try { await fs.rename(existingFile,quarantine); }
            catch(renameError){ throw new RunnerError(`Falha ao isolar arquivo corrompido: ${existingFile}`,{code:'CORRUPT_FILE_QUARANTINE_FAILED',cause:renameError}); }
          }
          await this.state.appendError({scope:'VERIFY',position,status:'CORRUPT_EXISTING_FILE',message:String(error?.message||error),quarantine});
          existingFile=null;
        }
      }
      if(!status){
        let lastProgressBucket=-1;const executeDownload=async()=>{attempts++;return await this.downloader.download({mediaUrl:lesson.videoUrl,refererUrl:lesson.pageUrl||this.workPage.url,paths,signal:this.shutdown.signal,onProgress:p=>{this.runtimeStats.recordDownloadProgress({speed:p.speedText});const bucket=Math.floor((p.percent||0)/5);if(bucket!==lastProgressBucket){lastProgressBucket=bucket;this.progressSink?.(`[DOWNLOAD ${position}/${this.total}] ${Number(p.percent||0).toFixed(1)}%${p.speedText?` @ ${p.speedText}`:''}${p.eta?` ETA ${p.eta}`:''}`);}}});};
        let dl=await executeDownload();
        let refreshAttempts=0;
        while(!dl.ok){
          downloadFailure=dl;
          if(shouldRefreshSignedMedia(dl,lesson) && refreshAttempts<this.limits.mediaRefreshRetries){
            refreshAttempts++;this.runtimeStats.recordMediaRefresh();
            const previousMediaUrl=lesson.videoUrl;
            await this.logger.log('RECOVERY','Refreshing signed media URL',{position,attempt:`${refreshAttempts}/${this.limits.mediaRefreshRetries}`,failureCode:dl.failureCode||dl.kind});
            const refreshedPage=await this.browser.refreshSameLesson(this.workPage);
            if(!refreshedPage)break;
            this.workPage=refreshedPage;
            const refreshed=await this.browser.inspectLesson(this.workPage);
            if(refreshed.currentPosition!==position)throw new RunnerError('Refresh de mídia mudou a posição da aula.',{code:'MEDIA_REFRESH_POSITION_CHANGED'});
            if(!refreshed.videoUrl)break;
            ...
            lesson=refreshed;
            dl=await executeDownload();
            continue;
          }
          break;
        }
        if(!dl.ok){
          downloadFailure=dl;status=dl.kind==='DRM'?'DRM_PROTECTED':'DOWNLOAD_FAILED';
          ...
          outputFile=dl.finalPath;
          try{validation=await this.downloader.validateVideo(outputFile,{signal:this.shutdown.signal});status='DOWNLOADED';await this.logger.log('VERIFY',`ffprobe OK — ${Math.round(validation.duration)}s`,{position,size:validation.size,codec:validation.codec});}
          catch(error){
            status='VERIFY_FAILED';
            let quarantine=null;
            try{quarantine=await this.downloader.quarantineCorrupt(outputFile);}catch{}
            await this.state.appendError({scope:'VERIFY',position,status,message:String(error?.message||error),outputFile,quarantine});
            outputFile=null;
          }
        }
      }
    ...
    if(RETRYABLE_FAILURE_STATUSES.has(status)){
      ...
      await this.state.setWorkPage(lesson.pageUrl||this.workPage.url);
      await this.logger.log('RETRYABLE',`Position ${position} remains pending`,{status});
      return {status,skipped:false,retryable:true,retryError:{code:status,failureCode:downloadFailure?.failureCode||null},failureCode:downloadFailure?.failureCode||null,page:this.workPage,lesson,outputFile:null,validation:null};
    }

    if(repair && existing){
      // Manifest is append-only and already owns this position. Repair the file without adding a duplicate row.
