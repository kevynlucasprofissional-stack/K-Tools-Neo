...
test('runner info exposes version and actual CLI/install paths',async()=>{const info=await getRunnerInfo();assert.match(info.version,/^4\.2\.5$/);assert.match(info.cliPath,/src[\\/]cli\.mjs$/);assert.ok(info.installRoot);assert.match(info.packageJson,/package\.json$/);});
test('doctor includes runnerVersion, cliPath and installRoot',async()=>{const d=await runDoctor({config:{profileDir:'/p',outputRoot:'/o',chromePath:process.execPath,cdpEndpoint:'http://127.0.0.1:9222'},playwrightLoader:async()=>({chromium:{connectOverCDP(){}}}),fetchImpl:async()=>{throw new Error('off');}});assert.equal(d.runnerVersion,'4.2.5');assert.ok(d.cliPath);assert.ok(d.installRoot);});
