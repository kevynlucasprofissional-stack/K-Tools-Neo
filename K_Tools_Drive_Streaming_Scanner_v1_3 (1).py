...
class DriveTreeScanner:
    ...
    def run(self, resume: bool = True) -> dict:
        ...
        retry_round = 0
        workers = max(1, min(int(self.options.max_workers or 1), 50))
...
class ScannerApp(tk.Tk):
    ...
    def _build_ui(self) -> None:
        ...
        ttk.Combobox(speed_row, textvariable=self.workers_var, values=("1", "2", "4", "6", "8", "10", "12", "14", "16", "18", "50"), state="readonly", width=5).pack(side="left", padx=(8, 8))
    ...
    def start_scan(self) -> None:
        ...
        if not root or not out:
            ...
            return
        try:
            workers = max(1, min(int(self.workers_var.get()), 50))
        except ValueError:
            workers = 4
