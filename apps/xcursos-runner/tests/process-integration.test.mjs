...
test('subprocess timeout does not return control while the child is still running',async()=>{
  const { runProcess }=await import('../src/process.mjs');
  const root=await fs.mkdtemp(path.join(os.tmpdir(),'xc-proc-timeout-'));
  const marker=path.join(root,'alive.txt');
  const timeoutMs=400,killGraceMs=150;
  ...
  await assert.rejects(()=>runProcess(process.execPath,['-e',script],{timeoutMs,killGraceMs}),e=>e.code==='PROCESS_TIMEOUT');
  const elapsed=Date.now()-start;
  if(process.platform==='win32')assert.ok(elapsed>=timeoutMs-50,`Windows force-kill returned before timeout: ${elapsed}ms`);
  else assert.ok(elapsed>=timeoutMs+killGraceMs-50,`POSIX timeout returned before SIGTERM grace elapsed: ${elapsed}ms`);
